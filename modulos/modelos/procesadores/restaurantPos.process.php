...
if( !empty($_REQUEST["Accion"]) ){
    ...
    switch ($_REQUEST["Accion"]) {
        ...
        case 19: //aumentar o disminuir la cantidad de un producto
            $item_id=$obCon->normalizar($_REQUEST["item_id"]);
            ...
            $datos_item=$obCon->DevuelveValores("restaurante_pedidos_items", "ID", $item_id);
            ...
            $iva_nuevo=round(($datos_item["IVA"]/$divisor)*$cantidad_nueva,2);
            $subtotal=$datos_item["ValorUnitario"]*$cantidad_nueva;
            $total_nuevo=$subtotal+$iva_nuevo;
            $sql="UPDATE restaurante_pedidos_items SET Cantidad='$cantidad_nueva',Subtotal='$subtotal',IVA='$iva_nuevo', Total='$total_nuevo' WHERE ID='$item_id' ";
            $obCon->Query($sql);
