...
if(!empty($_REQUEST['idNota'])){
        ...
        $idComprobante=$obVenta->normalizar($_REQUEST['idNota']);
}

	
print("<html>");
print("<head>");
$css =  new CssIni("Anular Nota Contable");

print("</head>");
print("<body>");
    
    include_once("procesadores/AnularNota.process.php");
    
    $css->CabeceraIni("Anular Nota Contable"); //Inicia la cabecera de la pagina
    ...
    /////
    /////	
    $css->CrearImageLink("notascontables.php", "../images/anular.png", "_self",200,200);
    ...
    if(!empty($_REQUEST["TxtidComprobante"])){
        $css->CrearTabla();
        $css->CrearFilaNotificacion("Se ha registrado la anulacion de la nota en el registro No. $_REQUEST[TxtidComprobante]</a>",16);
        $css->CerrarTabla();
    }
    ...
    if(!empty($idComprobante)){
        ...
        $css->CrearTabla();
            ...
            $css->ColTabla($DatosNota["Total"], 1);
            $css->ColTabla($DatosNota["Detalle"], 1);
