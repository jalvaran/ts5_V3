...
class InformesFinancieros extends Tabla{
    
    public function EstadosResultados_PDF($TipoReporte,$FechaInicial,$FechaFinal,$FechaCorte,$idEmpresa,$CentroCosto,$Vector ) {
        if($idEmpresa=="ALL"){
            $idEmpresaEncabezado=1;
        }
        if($TipoReporte=="Corte"){
            $FechaFinal=$FechaCorte;
            $FechaReporte="A Corte $FechaCorte";
        }else{
            $FechaReporte="Del $FechaInicial al $FechaFinal";
        }
        
        $this->PDF_Ini("Estado de Resultados", 8, "");
    ...
    public function ArmeTemporalSubCuentas($TipoReporte,$FechaFinal,$FechaInicial,$CentroCostos,$EmpresaPro,$Vector){
        ...
        $sql="SELECT `CuentaPUC` AS Cuenta,NombreCuenta AS NombreCuenta ,sum(`Neto`) as TotalCuenta FROM `librodiario` $Condicion  GROUP BY `CuentaPUC` ORDER BY `CuentaPUC`";
        ...
        while($DatosMayor=$this->obCon->FetchArray($Consulta)){
            if($DatosMayor["Cuenta"]>0){
                ...
                if($TipoReporte=="Corte"){
                    ...
                    $SaldoAnterior=$this->obCon->Sume("librodiario", "Neto", "WHERE Fecha<'$FechaInicial' AND `CuentaPUC`='$Cuenta'");
                }
