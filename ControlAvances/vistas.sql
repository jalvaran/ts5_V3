...
CREATE VIEW vista_resumen_restaurante_turno_actual AS
    ...
    SELECT t1.idProductosVenta,t1.Nombre,
        ...
        (SELECT IFNULL((SELECT SUM(Cantidad) FROM traslados_items ti WHERE ti.Referencia=t1.Referencia AND Destino='$SedeActual' AND idCierre='0'),0)) as TrasladosRecibidos,
        (SELECT IFNULL((SELECT SUM(Cantidad) FROM traslados_items ti WHERE ti.Referencia=t1.Referencia AND Destino<>'$SedeActual' AND idCierre='0' AND Estado='PREPARADO'),0)) as TrasladosEnviados,
