...
class PrintPos extends ProcesoVenta{
    public function EncabezadoComprobantesPos($handle,$Fecha, $idEmpresa){
        ...
        fwrite($handle, chr(27). chr(100). chr(1));// SALTO DE LINEA
        fwrite($handle,$Fecha); // ESCRIBO RAZON SOCIAL
        fwrite($handle, chr(27). chr(100). chr(1));// SALTO DE LINEA
    ...
    $sql="SELECT * FROM librodiario 
                WHERE Tipo_Documento_Intero='CompEgreso' AND idCierre='$idCierre' AND Debito>0";
    ...
    public function ComprobanteIngresoPOS($idIngreso,$COMPrinter,$Copias){
        ...
        $idEmpresa=1;
        $this->EncabezadoComprobantesPos($handle, $DatosIngreso["Fecha"], $idEmpresa);
...
fwrite($handle, chr(27). chr(100). chr(1));// SALTO DE LINEA
    ...
    public function ImprimeTraslado($idTraslado,$Copias) {
        $DatosImpresora=$this->DevuelveValores("config_puertos", "ID", 1);   
        if($DatosImpresora["Habilitado"]<>"SI"){
            return;
        }
    ...
    fwrite($handle, chr(27). chr(100). chr(1));//salto de linea
    fwrite($handle,"TOTAL ENTREGA        ".str_pad("$".number_format($DatosCierre["TotalEntrega"]+$TotalOtrosImpuestos+$TotalInteresesSisteCredito+$TotalAnticiposRecibidos+$AbonosAcuerdoOtrosMetodos+$TotalFacturasNegativas+$InteresesAcuerdoEfectivo+$InteresesAcuerdoOtrosMetodos+$AbonosAnticiposEncargosEfectivo+$AbonosAnticiposEncargosOtrosMetodos),20," ",STR_PAD_LEFT));
